  ...
  private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene) {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
            ...
            if (mesh->mTextureCoords[0]) { // right now we care only about the
                                           // first set of texture coords
                                           // (find out why)
                glm::vec2 vec;
    ...
    vector<Texture> loadMaterialTextures(aiMaterial *material,
                                         ...
                                         string typeName) {
        vector<Texture> textures;
        for (unsigned int i = 0; i < material->GetTextureCount(textureType);
             i++) {
             aiString str;
